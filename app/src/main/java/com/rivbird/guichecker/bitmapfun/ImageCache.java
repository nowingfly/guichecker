...
package com.rivbird.guichecker.bitmapfun;
...
public class ImageCache {
    ...
    public static ImageCache getInstance(android.app.FragmentManager fragmentManager, ImageCacheParams cacheParams) {

        // Search for, or create an instance of the non-UI RetainFragment
        final RetainFragmentApp mRetainFragment = findOrCreateRetainFragmentApp(fragmentManager);
    ...
    public static File getDiskCacheDir(Context context, String uniqueName) {
        ...
        final String cachePath = Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())
                        || !isExternalStorageRemovable() ? (getExternalCacheDir(context) != null ? getExternalCacheDir(
                        context).getPath() : context.getCacheDir().getPath()) : context.getCacheDir().getPath();
    ...
    private static RetainFragmentApp findOrCreateRetainFragmentApp(android.app.FragmentManager fm) {
        // BEGIN_INCLUDE(find_create_retain_fragment)
        // Check to see if we have retained the worker fragment.
        RetainFragmentApp mRetainFragment = (RetainFragmentApp) fm.findFragmentByTag(TAG);

        // If not retained (or first time running), we need to create and add
        // it.
        if (mRetainFragment == null) {
            mRetainFragment = new RetainFragmentApp();
            fm.beginTransaction().add(mRetainFragment, TAG).commitAllowingStateLoss();
        }
     ...
     * changes. It will be used to retain the ImageCache object.
     */
    public static class RetainFragmentApp extends android.app.Fragment {
        private Object mObject;

        /**
         * Empty constructor as per the Fragment documentation
         */
        public RetainFragmentApp() {}
